...
app.post("/Add", function(req, res){
    ...
    var data = {name, email, provider_contact, state, city, location, beds, oxygen, others, helpful, not_helpful, verified} = req.body
    firebase.database().ref(req.body.state + "/"+req.body.city+"/"+req.body.location).set({name, email, provider_contact, state, city, location, beds, oxygen, others, helpful, not_helpful, verified});
...
function validate(data){
    ...
    if (!data.name.match(nameRegex) || !data.city.match(nameRegex) || !data.state.match(nameRegex)){
        ...
        return false
    else if(isNaN(data.beds) || isNaN(data.oxygen) || isNaN(data.provider_contact))
        return false
